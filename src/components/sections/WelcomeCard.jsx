...
const WelcomeCard = ({ name }) => {
  ...
  const totalItems = JsonReading.length; // Usa o total real (150)
  
  const percentage =
    totalRead === 0
      ? 0
      : totalRead >= totalItems
        ? 100
        : Math.min(99, Math.ceil((totalRead / totalItems) * 100));

  // função para definir a mensagem dinamicamente
  const getProgressMessage = () => {
    if (totalRead === 0) return 'Vamos começar a sua jornada?';
    if (totalRead >= totalItems) return 'Glória a Deus! Você concluiu o Plano!';
    ...
    <FadeUp delay={0.3}>
      <section className="mx-auto -mt-36 flex w-full max-w-[1440px] flex-col gap-6 px-6 md:px-8 lg:flex-row">
        ...
        <Card className="w-full flex-1 gap-6 md:gap-10">
          {/* Adicione o SVG Dourado do Figma na pasta public/assets/SVG e troque aqui se necessário */}
          <img
            src="/welcome-icon.svg"
            alt=""
            className="w-32 shrink-0 xl:w-44"
          />

          <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
            <h2 className="font-title text-3xl font-bold text-white xl:text-4xl">
              Olá, {name}
            </h2>
        ...
        <Card className="w-full flex-1 gap-10 xl:gap-32">
          <div className="flex h-full w-full flex-1 flex-col items-center justify-center py-2 text-center md:items-start md:text-left">
            <h2 className="font-title text-3xl font-bold leading-tight text-white xl:text-3xl">
